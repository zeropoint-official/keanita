...
const HIGHLIGHTS = [
  { emoji: '🎨', label: 'Διασκεδαστικά δημιουργικά εργαστήρια' },
  { emoji: '🍓', label: 'Υγιεινά σνακ Keanita' },
  { emoji: '🏅', label: 'Κέρδισε KP και δώρα' },
];
...
function formatLongDate(dateStr: string) {
  ...
  return d.toLocaleDateString('el', { weekday: 'long', day: 'numeric', month: 'long' });
}
...
export default function EventDetailScreen() {
  ...
  if (!event) {
    return (
      <View style={{ backgroundColor: '#FAFAF7', minHeight: '100dvh' }}>
        <ScreenHeader title="Εκδήλωση" />
        <EmptyState emoji="📅" title="Η εκδήλωση δεν βρέθηκε" />
      </View>
    );
  }
  ...
  const handleRsvp = () => {
    if (rsvped) return;
    setRsvped(true);
    earn(10, `Δήλωση συμμετοχής — ${event.title}`, 'event');
  };

  const infoRows: { icon: IconSymbolName; label: string; value: string }[] = [
    { icon: 'calendar', label: 'Ημερομηνία', value: formatLongDate(event.date) },
    { icon: 'clock.fill', label: 'Ώρα', value: event.time },
    { icon: 'mappin.and.ellipse', label: 'Τοποθεσία', value: event.location },
  ];
      ...
      <View style={{ paddingBottom: 160 }}>
        ...
        <View style={{ backgroundColor: '#FAFAF7', borderTopLeftRadius: 32, borderTopRightRadius: 32, marginTop: -28, paddingLeft: 24, paddingRight: 24, paddingTop: 24 }}>
          <View style={{ alignSelf: 'flex-start', borderRadius: 999, paddingLeft: 12, paddingRight: 12, paddingTop: 4, paddingBottom: 4, backgroundColor: event.accentColor }}>
            <Text style={{ ...Fonts.bodyBold, fontSize: 10, color: '#FFFFFF' }}>{TYPE_LABEL[event.type]}</Text>
          </View>
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 8 }}>
            ...
            <Text style={{ ...Fonts.bodyBold, fontSize: 12, color: '#F5A623', marginLeft: 4 }}>Κέρδισε 10 KP με τη δήλωση συμμετοχής</Text>
          </View>
          ...
          {/* About */}
          <Text style={{ ...Fonts.displayMedium, fontSize: 18, color: '#2D2D3A', marginTop: 24, marginBottom: 8 }}>Σχετικά με την εκδήλωση</Text>
          <Text style={{ ...Fonts.body, fontSize: 14, color: '#8E8E9A', lineHeight: '24px' }}>{event.description}</Text>

          {/* Highlights */}
          <Text style={{ ...Fonts.displayMedium, fontSize: 18, color: '#2D2D3A', marginTop: 24, marginBottom: 12 }}>Τι να περιμένεις</Text>
          {HIGHLIGHTS.map((h, i) => (
      ...
      <View style={{ position: 'fixed', bottom: 76, left: '50%', transform: 'translateX(-50%)', width: '100%', maxWidth: 430, boxSizing: 'border-box', paddingLeft: 24, paddingRight: 24, paddingTop: 12, paddingBottom: 12, backgroundColor: '#FAFAF7', borderTop: '1px solid #F0F0EC', zIndex: 30 }}>
        {rsvped ? (
          <View style={{ borderRadius: 16, paddingTop: 14, paddingBottom: 14, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', backgroundColor: '#EEFBEE' }}>
            ...
            <Text style={{ ...Fonts.bodyBold, fontSize: 14, color: '#6BBF6A', marginLeft: 8 }}>Δηλώθηκες! +10 KP</Text>
          </View>
        ) : (
          <PrimaryButton label="Δήλωσε συμμετοχή" icon="checkmark" color={event.accentColor} onPress={handleRsvp} />
        )}
      </View>
    </View>
  );
